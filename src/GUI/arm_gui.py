...
tab1 = [[sg.Text('Welcome to the Robotic Arm Interface! Read the following instructions to interact with the arm:')],
        ...
        [sg.Text('Free-hand mode:\t'),
         sg.Button('Free-hand drawing mode', key='free_mode'), sg.Button('Exit Free-hand mode', key='exit_mode', disabled=True)]]
...
while True:
    ...
    if event == 'free_mode':
        ...
        serial_data = values['xpos'] + values['ypos']
        #print(serial_data)
        ...
        serial_data = values['xpos'] + values['ypos'] + values['vel']
        #print(serial_data)
