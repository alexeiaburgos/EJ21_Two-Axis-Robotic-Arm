...
void kinematics(double* source, double* destination, uint8_t num) {
  ...
  //num will act as the number of steps that will be done for each movement.
  ...
  while (1) {
    ...
    //call the move function to send the outputs to the motor driver for as many times as there are steps
    ...
    /*
    //for debugging purposes, print the coordinates:
...
*/
    ...
    if (bestDistance < .2*8*(num+1)*(num+1)) {
      break;
    }
  }
  ...
  if (Serial.available() >= 0) {
    ...
    if (moveMode == 0) {
      ...
      if (!strcmp(commandArray[0], "G00")) { //execute if the G00 command is the input
        ...
        kinematics(src, dst, 3); //can use 3 rather than 2 if not fast enough
        //Serial.print("Done!\n");
       ...
       // Serial.print(speed);
        ...
        //do nothing, or test the movement:
        //testMovement(); 
      }
        ...
        kinematics(src, dst, 2); //can change to 2 if needed
        //Serial.print("Done!\n");
